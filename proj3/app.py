...
def main():
    ...
    if LOGGED_IN != True:
        ...
        global constant_fk2d, Shear_velocity, multiplying_factor_3d

        # first slider for constant_fk2d
        constant_fk2d = st.sidebar.slider('constant_fk2d:', min_value=0.0,
                                          max_value=1.0, value=0.75)
        st.sidebar.write("\n")

        # second slider for multiplying_factor_3d
        multiplying_factor_3d = st.sidebar.slider('multiplying factor',
                                                  min_value=0.0, max_value=1.0, value=0.5)
        st.sidebar.write("\n")

        # third slider for Shear_velocity
        Shear_velocity = st.sidebar.slider(
            'velocity:', min_value=0.0, max_value=100.0, value=2.6**3)
        st.sidebar.write("\n")

        filter_options_list = ('1. C', '2. S', '3. A', '4. C & S',
                               '5. C & A', '6. S & A', '7. C & S & A', '8. all combine')

        filter = st.selectbox(
            "Chose Filtering Method From Below:", filter_options_list)
        st.write("You Selected:", filter.split('.')[1])

        # bool var to check if required value for filtering is given or not
        # required_value_state=False
        # selected filter option in numeric value
        tch = int(filter.split('.')[0])

        # to create a space  between layouts
        st.markdown('####')
        global Lambda, corr, SNR, k, name
        ...
        # else:
        #     pass
            # st.warning('Please enter correct choice...',"⚠️")
            # required_value_state = False

        # to create a space  between layouts
        st.markdown('#')
        replacement_method_list = ('1. previous point', '2. 2*last-2nd_last', '3. overall_mean',
                                   '4. 12_point_strategy', '5. mean of previous 2 point',
                                   '6. all seqential', '7. all parallel')

        # to get the replacement technique using selectbox
        replacement = st.selectbox(
            'Chose Replacement Method From Above:', replacement_method_list)
        st.write("Replacement Selected:", replacement.split('.')[1])
        sch = int(replacement.split('.')[0])
            ...
            for file in files:
                ...
                if tch == 1 or tch == 8:
                    ...
                    for i in iList:
                        ...
                        Corr_All(i, corr)
                        allfunction()
                        name = f"{input_file_name}_filtered_by_correlation{corr}_all_replacement_strategy_{i}"
                        ...
                        Corr_One(i, corr)
                        allfunction()
                        name = f"{input_file_name}_filtered_by_correlation{corr}_individual_replacement_strategy_{i}"
                        write_timestamp_to_file(name)
                        name = add_front_name(name, i)
                        store()
                    ...
                    if tch == 2 or tch == 8:
                        ...
                        for i in iList:
                            ...
                            SNR_All(i, SNR)
                            allfunction()
                            name = f"{input_file_name}_filtered_by_SNR{SNR}_all_replacement_strategy_{i}"
                            write_timestamp_to_file(name)
                            name = add_front_name(name, i)
                            store()
                            ...
                            SNR_One(i, SNR)
                            allfunction()
                            name = f"{input_file_name}_filtered_by_SNR{SNR}_individual_replacement_strategy_{i}"
                            write_timestamp_to_file(name)
                            name = add_front_name(name, i)
                            store()
                    ...
                    if tch == 3 or tch == 8:
                        ...
                        for i in iList:
                            ...
                            allfunction()
                            name = f"{input_file_name}_filtered_by_acceleration_1.5_all_replacement_strategy_{i}"
                            write_timestamp_to_file(name)
                            name = add_front_name(name, i)
                            store()
                            ...
                            allfunction()
                            name = f"{input_file_name}_filtered_by_acceleration_1.5_individual_replacement_strategy_{i}"
                            write_timestamp_to_file(name)
                            name = add_front_name(name, i)
                            store()
                    ...
                    if tch == 4 or tch == 8:
                        ...
                        for i in iList:
                            ...
                            SNR_All(i, SNR)
                            allfunction()
                            name = f"{input_file_name}_filtered_by_correlation{corr}_SNR{SNR}_all_replacement_strategy_{i}"
                            write_timestamp_to_file(name)
                            name = add_front_name(name, i)
                            store()
                            ...
                            SNR_One(i, SNR)
                            allfunction()
                            name = f"{input_file_name}_filtered_by_correlation{corr}_SNR{SNR}_individual_replacement_strategy_{i}"
                            write_timestamp_to_file(name)
                            name = add_front_name(name, i)
                            store()
                    ...
                    if tch == 5 or tch == 8:
                        ...
                        for i in iList:
                            ...
                            update_acceleration_all_at_time(i)
                            allfunction()
                            name = f"{input_file_name}_filtered_by_SNR{SNR}_Acceleration_1.5_all_replacement_strategy_{i}"
                            write_timestamp_to_file(name)
                            name = add_front_name(name, i)
                            store()
                            ...
                            update_acceleration_all_at_time(i)
                            allfunction()
                            name = f"{input_file_name}_filtered_by_SNR{SNR}_acceleration_1.5_individual_replacement_strategy_{i}"
                            write_timestamp_to_file(name)
                            name = add_front_name(name, i)
                            store()
                    ...
                    if tch == 6 or tch == 8:
                        ...
                        for i in iList:
                            ...
                            update_acceleration_all_at_time(i)
                            allfunction()
                            name = f"{input_file_name}_filtered_by_correlation{corr}_Acceleration_1.5_all_replacement_strategy_{i}"
                            write_timestamp_to_file(name)
                            name = add_front_name(name, i)
                            store()
                            ...
                            update_acceleration_all_at_time(i)
                            allfunction()
                            name = f"{input_file_name}_filtered_by_correlation{corr}_acceleration_1.5_individual_replacement_strategy_{i}"
                            write_timestamp_to_file(name)
                            name = add_front_name(name, i)
                            store()
                    ...
                    if tch == 7 or tch == 8:
                        ...
                        for i in iList:
                            ...
                            update_acceleration_all_at_time(i)
                            allfunction()
                            name = f"{input_file_name}_filtered_by_correlation{corr}_SNR{SNR}_Acceleration_1.5_all_replacement_strategy_{i}"
                            write_timestamp_to_file(name)
                            ...
                            update_acceleration_all_at_time(i)
                            allfunction()
                            name = f"{input_file_name}_filtered_by_correlation{corr}_SNR{SNR}_acceleration_1.5_individual_replacement_strategy_{i}"
                            write_timestamp_to_file(name)
                            name = add_front_name(name, i)
                            store()
                ...
                # st.area_chart(input_data)
                # st.bar_chart(input_data['U', 'V', 'W'])
                # st.bokeh_chart(input_data)
                # st.plotly_chart(input_data)
                # st.graphviz_chart(input_data)
